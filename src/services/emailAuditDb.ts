...
class EmailAuditDatabase {
  ...
  public recordDispatch(data: {
    ...
    const newRecord: EmailAuditRecord = {
      ...
      to: data.to,
      from: data.from || process.env.EMAIL_FROM || 'Sessão Certa <nao-responda@sessaocerta.shop>',
  ...
  public updateStatusFromWebhook(webhookPayload: {
    ...
    if (!record) {
      ...
      record = {
        ...
        emailId,
        to: recipient,
        from: webhookPayload.data.from || process.env.EMAIL_FROM || 'Sessão Certa <nao-responda@sessaocerta.shop>',
